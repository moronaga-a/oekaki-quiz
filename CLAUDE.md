...
- [ ] Controller 拡張
  - [ ] プレイヤー参加処理
  - [ ] プレイヤー名バリデーション（必須、1-10文字）
  - [ ] セッション管理（player_id, room_id保存）
...
- [ ] 退出ロジック実装
  - [ ] WebSocket切断時の自動退出
  - [ ] ホスト変更ロジック（Room#remove_player内）
  - [ ] 手動退出ボタン実装

### フェーズ4: お絵描き機能実装

#### Canvas 描画機能
- [ ] Canvas Controller 作成
  - [ ] Canvas初期化処理
  - [ ] ペン描画機能（黒のみ、線幅調整）
  - [ ] 消しゴム機能
  - [ ] リセット機能（全体クリア）
